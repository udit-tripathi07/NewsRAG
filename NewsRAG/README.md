...
## Notes on the NewsAPI integration

- NewsAPI's `sources`/`domains` filters were unreliable for several of the listed publishers on the free tier, so **Newsroom** fetches broadly (`q=India`, date-scoped) and filters down to the chosen publication client-side by matching `source.name`. A debug expander ("actual source names found") is shown after loading so you can adjust the match terms in `NEWSPAPER_NAME_MATCHES` if a publisher isn't matching correctly on your account.
- Only "The Hindu" and "Times of India" were verified against a live account when the matching list was built — double check the rest against your own results.
- **Topic Radar** searches NewsAPI's `/everything` endpoint directly by keyword across all indexed publishers, sorted by relevancy.
- Date pickers are capped at the last 10 days (`MAX_DAYS_BACK`) — adjust this constant if your NewsAPI plan allows a wider range.
